...
function AdminApp({ onLogout }) {
  ...
  const reloadAll = () => {
    setLoading(true);
    Promise.all([api?.getCategories(), api?.getProducts(), api?.getPaymentMethods(), api?.getOrders(), api?.getSettings()])?.then(([c, p, pm, o, s]) => { setCategories(c); setProducts(p); setPaymentMethods(pm); setOrders(o); setSettings(s); })?.finally(() => setLoading(false));
  };
  useEffect(reloadAll, []);
...
function emptyProduct(categories) {
  return { name: "", description: "", image: "", categoryId: categories?.[0]?.id || "", active: true, featured: false, isNew: false, identifierType: "player_id", packages: [] };
}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface border border-border w-full sm:max-w-2xl sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-xs text-muted mb-1.5">ما الذي يحتاجه العميل لتنفيذ الشحن؟</label>
            <select value={p?.identifierType || "player_id"} onChange={(e) => set({ identifierType: e?.target?.value })} className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-gold">
              <option value="player_id">🆔 معرّف اللاعب (ID) فقط</option>
              <option value="email_password">📧 بريد إلكتروني + كلمة مرور الحساب</option>
              <option value="none">لا يحتاج أي معرّف</option>
            </select>
          </div>
          ...
          {orders?.map((o) => (
            <div key={o?.id} className="bg-surface border border-border rounded-xl p-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 text-[11px] text-muted">
                {o?.customerName && <span>العميل: {o?.customerName}</span>}
                ...
                {o?.playerId && <span>🆔 المعرّف: {o?.playerId}</span>}
                {o?.accountEmail && <span>📧 البريد: {o?.accountEmail}</span>}
                {o?.accountPassword && (
                  <span className="flex items-center gap-1">
                    🔑 كلمة المرور: {revealed?.[o?.id] ? o?.accountPassword : "••••••••"}
                    <button onClick={() => setRevealed({ ...revealed, [o?.id]: !revealed?.[o?.id] })} className="text-gold underline">
                      {revealed?.[o?.id] ? "إخفاء" : "إظهار"}
                    </button>
                  </span>
                )}
